    ...
    def load_dataset(self):
        ...
        for defect_type in defect_types:
            if defect_type == 'good':
                ...
                img_paths.sort()
                gt_paths.sort()
                # 只添加有对应ground truth的图像
                min_len = min(len(img_paths), len(gt_paths))
                img_tot_paths.extend(img_paths[:min_len])
                gt_tot_paths.extend(gt_paths[:min_len])
                tot_labels.extend([1] * min_len)
                tot_types.extend([defect_type] * min_len)
